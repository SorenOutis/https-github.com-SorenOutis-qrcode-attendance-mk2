...
<template>
    <div v-if="atRiskStudents.length > 0" class="overflow-hidden rounded-[1.5rem] border border-rose-100 dark:border-rose-900/30 bg-white/60 dark:bg-zinc-900/40 backdrop-blur-xl shadow-sm">
            ...
            <div class="divide-y divide-rose-100/30 dark:divide-rose-900/30 max-h-64 overflow-y-auto">
                ...
                >
                    <div class="flex flex-col min-w-0">
                        <span class="text-xs font-bold text-zinc-900 dark:text-white truncate" :title="student.name">{{ student.name }}</span>
                        <span class="text-[9px] font-bold text-zinc-400 uppercase tracking-widest mt-0.5">{{ student.student_number }}</span>
                    </div>
                    <div class="flex flex-col items-end shrink-0">
                        <span class="text-[11px] font-black text-rose-600 dark:text-rose-400 tabular-nums">{{ student.attendance_percentage }}%</span>
                        <span class="text-[8px] font-bold text-zinc-400 uppercase tracking-widest leading-none mt-1">Rate</span>
                    </div>
                </div>
            </div>
        </div>
